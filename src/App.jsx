...
function App() {
  ...
  const i18n = {
    en: {
      ...
      portfolio:[{title:'Fashion E-commerce',category:'Online Store',description:'Full platform + payments',gradient:'from-purple-500 to-pink-500'},
                 {title:'Tech Startup',category:'Corporate Website',description:'Modern institutional site',gradient:'from-blue-500 to-cyan-500'},
    ...
    pt: {
      ...
      portfolio:[{title:'E-commerce de Moda',category:'Loja Virtual',description:'Plataforma + pagamentos',gradient:'from-purple-500 to-pink-500'},
                 {title:'Tech Startup',category:'Site Corporativo',description:'Site institucional moderno',gradient:'from-blue-500 to-cyan-500'},
        ...
        <div className="container mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {t.services.map((srv, idx)=>(
              <Card key={idx} className={`bg-slate-900/50 border-slate-800 hover:border-cyan-500/50 h-full ${activeService===idx?'ring-2 ring-cyan-500/50':''}`}
      ...
      <footer className="py-12 px-4 bg-slate-950 border-t border-slate-800">
        <div className="container mx-auto grid md:grid-cols-4 gap-8 mb-8">
          ...
          <div>
            ...
            <ul className="space-y-2 text-slate-400 text-sm">{t.footer.servicesLinks.map((s,i)=>(<li key={i} className="hover:text-cyan-400"> {s} </li>))}</ul>
          </div>
          <div>
            <h4 className="font-semibold mb-4 text-white">{t.footer.company}</h4>
            <ul className="space-y-2 text-slate-400 text-sm">{t.footer.companyLinks.map((s,i)=>(<li key={i} className="hover:text-cyan-400"> {s} </li>))}</ul>
          </div>
          <div>
