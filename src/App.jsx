...
function App() {
  ...
  const i18n = {
    en: {
      ...
      // ⬇️ UPDATED: first two items use images instead of gradients
      portfolio:[
        {
    ...
    pt: {
      ...
      // ⬇️ ATUALIZADO: dois primeiros cards com imagem
      portfolio:[
        {
        ...
        <div className="container mx-auto grid md:grid-cols-2 gap-6">
          {t.portfolio.map((p, i)=>(
            <Card key={i} className="bg-slate-900/50 border-slate-800 overflow-hidden h-full hover:border-cyan-500/50 transition-all">
              {/* ⬇️ se houver p.image, usa a foto; senão, usa o gradiente existente */}
              {p.image ? (
                <div
